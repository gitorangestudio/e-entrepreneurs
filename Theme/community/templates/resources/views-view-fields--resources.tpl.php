...
// Check user access according to the taxonomy access lit module
global $user;
$grants = tac_lite_node_grants($user, 'view');


if(isset($row->_field_data['nid']['entity']->field_resource_access['und'][0])){
	$access_tid = $row->_field_data['nid']['entity']->field_resource_access['und'][0]['tid'];
	//dsm($access_tid);
	//dsm($grants);
	if (in_array($access_tid, $grants['tac_lite'])) {
		$access_flag = true;
	}else{
		$access_flag = false;
	}
